...
def initialize_task():
    ...
    tasks['robot1']['apples_goal'] = random.randint(1,2)
    tasks['robot1']['oranges_goal'] = random.randint(1,2)

    #robot 2 tasked with oranges
    tasks['robot2']['apples_goal'] = random.randint(1,2)
    tasks['robot2']['oranges_goal'] = random.randint(1,2)
...
def report_hazards(robot,position):
    ...
    print(f"Hazard reported at {position}")

def get_tasks(robot):
    """Report progress on each task"""
    robot_id = f"robot{robot['id']}"
    return{
        'apples_goal': tasks[robot_id]['apples_goal'],
        'apples_collected': tasks[robot_id]['apples_collected'],
        'oranges_goal': tasks[robot_id]['oranges_goal'],
        'oranges_collected': tasks[robot_id]['oranges_collected'],
        'hazards_reported': tasks[robot_id]['hazards_reported'],
    }
